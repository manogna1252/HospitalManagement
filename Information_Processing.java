...
public class Information_Processing {
	...
	static ResultSet rs;
	static String str, staff_name, job_title, dob, address, department, phone_number, gender;
	int action;
	int staff_id;
		...
		case 1:
			...
			switch (staff_selection){
				case 1: 
					...
					String str1 = "INSERT INTO Doctor(doctor_id, professional_title, department, test_id) VALUES (?, ? ,?, ?)";
					try {
			...
			case 2: 
					...
					professional_title = reader.nextLine();
					str = "INSERT INTO Staff(staff_id, name, dob, gender, job_title, phone, address) VALUES (?, ?, ?, ?, ?, ?, ?)";
					String str1 = "INSERT INTO Nurse(nurse_id, department) VALUES (?, ? )";
					try {
					stmt = conn.prepareStatement(str);
					stmt.setInt(1, staff_id);
					stmt.setString(2, staff_name);
